...
export const fetchPaymentPrefixData = (req, res) => {
  const q =
    "select distinct sale_payment_in_prefix , max(CAST(sale_payment_in_prefix_no as SIGNED)) as sale_payment_in_prefix_no from sale_module group by sale_payment_in_prefix ORDER By sale_payment_in_prefix = 'PaymentIn' DESC ;";
  db.query(q, (err, data) => {
    if (err) return res.status(500).json(err);
    return res.status(200).json(data);
  });
